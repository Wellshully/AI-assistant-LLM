...
def parse_event_request(user_input: str, now: datetime):
    ...
    prompt = f'你是一個日曆助理，請根據使用者說的話解析出一個 Google Calendar 行程，並輸出符合 JSON 格式的事件物件，欄位包括：- summary: 簡短活動描述 -is_delete: true 表示刪除，false 表示新增 - start: ISO 格式（例如 2025-06-25T15:00:00）- end: ISO 格式（例如 2025-06-25T16:00:00）- timeZone: 請固定為 "Asia/Taipei"今天是 {today_str}，請根據這個時間推理出像「明天」、「下週一」這些詞指的是哪一天與幾點。如果資訊模糊或者不確定是新增或刪除回覆"None"。不要補充說明，只輸出 JSON 格式物件。使用者說：「{user_input}」'
